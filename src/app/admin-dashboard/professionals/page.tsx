...
export default function AdminProfessionalsPage() {
  ...
  const fetchApplications = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/admin/professionals/applications?status=${statusFilter}&limit=100`);
      if (!res.ok) throw new Error('فشل في تحميل الطلبات');
      const data = await res.json();
      setApplications(data.professionals || []);
    } catch (e) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/professionals/${deleteId}`, { method: 'DELETE' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">المحترفون</h1>
          <p className="text-muted text-sm mt-1">مراجعة واعتماد طلبات المحترفين</p>
        </div>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="البحث بالاسم، التخصص، البريد، المدينة..."
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center text-primary font-bold text-lg overflow-hidden">
                          {app.user.avatar ? (
                            <img src={app.user.avatar} alt="" className="w-full h-full object-cover" />
                          ) : (
                            <User className="w-6 h-6" />
                          )}
                        </div>
                        <div>
                          <h3 className="font-bold text-foreground">{app.user.name || 'محترف'}</h3>
                          <p className="text-sm text-primary">{app.title || '—'}</p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Link
                          href={`/admin-dashboard/professionals/${app.id}`}
                          aria-label="مراجعة التفاصيل"
                    ...
                    {app.bio && (
                      <p className="text-sm text-muted mt-3 line-clamp-2">{app.bio}</p>
                    )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
                      ...
                      {app.country && (
                        <div className="flex items-center gap-2 text-sm text-muted">
                          <MapPin className="w-4 h-4 text-muted" />
                          <span className="truncate">{app.country.name}</span>
                        </div>
                      )}
                      {app.phone && (
                        <div className="flex items-center gap-2 text-sm text-muted">
      ...
      <ConfirmModal
        isOpen={!!deleteId}
        title="حذف الملف المهني"
        message="هل أنت متأكد من حذف هذا الملف المهني؟ سيتم حذف جميع البيانات المرتبطة به."
