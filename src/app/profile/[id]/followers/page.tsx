        ...
        <div className="max-w-xl mx-auto px-4">
          <Card className="mb-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-lg font-bold text-foreground">المتابعون</h1>
            </div>
          </Card>
          ...
          {loading ? (
            ...
            <Card className="text-center py-10">
              <p className="text-muted">حدث خطأ أثناء جلب المتابعين.</p>
            </Card>
          ) : followers.length === 0 ? (
            <EmptyState
              icon={Users}
              title="لا يوجد متابعون"
              description="هذا المستخدم ليس لديه متابعون بعد."
            />
          ) : (
            <div className="space-y-3">
              {followers.map((follower) => (
                <Link key={follower.id} href={`/profile/${follower.id}`}>
                  <Card className="flex items-center gap-3 hover:border-primary/30 transition-colors">
                    <img
                      src={follower.avatar || '/logo/favicon.svg'}
                      alt={follower.name || 'مستخدم'}
